...
const TAG_SPEED_SLOW = 0.55;
const TAG_SPEED_SLOW_HOLD = 3;
const TAG_SPEED_SLOW_FADE = 1;
const BOT_COUNT = 4;
...
export class Game {
  ...
  resize() {
    ...
    const responsivePadding = Math.max(12, Math.min(ARENA_PADDING, rect.width * 0.06, rect.height * 0.06));
    const availableW = Math.max(48, rect.width - responsivePadding * 2);
    const availableH = Math.max(48, rect.height - responsivePadding * 2);
    const arenaW = Math.min(ARENA_MAX_WIDTH, availableW);
    const arenaH = Math.min(ARENA_MAX_HEIGHT, availableH);
  ...
  update(deltaTime, timestamp) {
    ...
    if (this.flashTimer > 0) {
      ...
      if (this.flashTimer <= 0) {
        this.callbacks.onFlash?.(false);
      }
    }

    if (this.tagCooldownLeft > 0) {
      this.tagCooldownLeft = Math.max(0, this.tagCooldownLeft - deltaTime);
    }
    ...
    for (const bot of this.bots) {
      updateBotAI(bot, this.entities, this.bounds, timestamp);
    }

    for (const entity of this.entities) {
      entity.updateSpeedBoost(deltaTime);
    }

    this.movePlayer(deltaTime);

    for (const bot of this.bots) {
      bot.moveTowardTarget(deltaTime, this.bounds);
    }
  ...
  movePlayer(deltaTime) {
    ...
    if (dx !== 0 || dy !== 0) {
      this.player.moveByInput(dx, dy, deltaTime, this.bounds);
    } else {
      this.player.moveTowardTarget(deltaTime, this.bounds);
    }
  }
  ...
  transferIt(from, to) {
    ...
    for (const entity of this.entities) {
      if (!entity.isIt) {
        entity.applySpeedBoost(TAG_SPEED_BOOST, TAG_SPEED_BOOST_HOLD, TAG_SPEED_BOOST_FADE);
      }
    }
    to.applySpeedBoost(TAG_SPEED_SLOW, TAG_SPEED_SLOW_HOLD, TAG_SPEED_SLOW_FADE);
  ...
  updateStatus() {
    if (this.player.isIt) {
      this.callbacks.onStatusChange?.('You are IT!', 'it');
    } else {
      this.callbacks.onStatusChange?.('RUN!', 'run');
    }
  }
